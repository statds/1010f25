...
No duplicates imply that `tailnum` is a valid primary key. To check for
...
The result shows how many flights lack corresponding aircraft
information.
...
This left join keeps every flight and adds aircraft details whenever a
matching `tailnum` exists. Unmatched rows contain missing values.

#### Example: Add Airline Names

```{r}
flights2 %>% left_join(airlines, by = "carrier")
```

Now each carrier code is replaced with a readable airline name, making
the dataset easier to interpret.
