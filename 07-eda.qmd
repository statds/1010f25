...
1. Plot the number of crashes by day and by hour. Do weekends/holidays
   or  evening show distinct patterns compared to weekdays?
1. Which borough recorded the most crashes? Are some boroughs missing
   altogether?
1. How many records have missing ZIP codes? Are the missing ones
   concentrated in certain boroughs?
1. How many crashes are missing latitude/longitude? What fraction of
   crashes with missing borough also lack coordinates?
1. Compare counts of missing values in `ON STREET NAME`, `CROSS STREET
   NAME`, and `OFF STREET NAME`. What does this suggest about where
   data collection is weaker?
1. How many crashes involved at least one person injured? At least one
   fatality? Show the distribution of injury counts.
1. What are the most common contributing factors for `VEHICLE 1`? Are
   there many “Unspecified” entries?
1. What are the most frequent vehicle categories in 
   `VEHICLE TYPE CODE 1`? Compare them with secondary vehicles
   (`VEHICLE TYPE CODE 2`).
1. Are there crashes with very high numbers of persons injured? How
   rare are these cases, and do they cluster in any borough?
