...
- `tmap` is specifically designed to make creation of thematic maps
  more convenient.
...
- Useful things to know:
  ...
  + `tm_shape()` where we provide the `sf` object (we could also
    provide an `SpatialPolygonsDataframe`).
	
  + `tm_polygons()` where we set the attribute variable to map, the
    break style, and a title.
...
**Animation** is a feature we need to find a dataset to demonstrate,
possibly through the NYC crash data.
